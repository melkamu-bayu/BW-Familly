...
def _to_out(unit: BusinessUnit, vehicle: Vehicle, category: BusinessCategory) -> VehicleOut:
    return VehicleOut(
        id=vehicle.id,
...
def list_vehicles(
    ...
    """`category` filters by BusinessCategory.code, e.g. ?category=MACHINES
    or ?category=VEHICLES. Omit it to get both (existing callers are
    unaffected). Single JOIN to BusinessCategory here -- not a per-row
    lookup -- so this stays O(1) queries regardless of how many vehicles
    or machines exist."""
    stmt = (
        select(Vehicle, BusinessUnit, BusinessCategory)
        .join(BusinessUnit, BusinessUnit.id == Vehicle.business_unit_id)
        .join(BusinessCategory, BusinessCategory.id == BusinessUnit.category_id)
        .where(BusinessUnit.deleted_at.is_(None))
    )
    if status:
        stmt = stmt.where(Vehicle.status == status)
    if category:
        stmt = stmt.where(BusinessCategory.code == category)
    rows = db.execute(stmt).all()
    return [_to_out(unit, vehicle, cat) for vehicle, unit, cat in rows]
...
def create_vehicle(
    ...
    db.commit()
    db.refresh(vehicle)
    return _to_out(unit, vehicle, category)
...
def get_vehicle(
    ...
    category = db.get(BusinessCategory, unit.category_id)
    return _to_out(unit, vehicle, category)
